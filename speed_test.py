...
def render_view(model_path, name, iteration, views, gaussians, pipeline, background, view_index):
    ...
    print(f"Warmup renders for view {view_index}")
    for _ in tqdm(range(500)):
        render(view, gaussians, pipeline, background)

    print(f"Real renders for view {view_index}")
    speed_loop = tqdm(range(500))
    deltas = 0
    for _ in speed_loop:
        pre_render = time.perf_counter()
        render(view, gaussians, pipeline, background)
        deltas += time.perf_counter() - pre_render

    print(f"Duration:\t{deltas}")
    print(f"FPS:\t\t{500 / deltas}")
